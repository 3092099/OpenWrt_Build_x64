...
#sed -i "s|DISTRIB_REVISION='LEDE*'|DISTRIB_REVISION='OpenWrt_1806_x64_主路由版 by GXNAS build @R$(date +%y.%m.%d)'|g" package/lean/default-settings/files/zzz-default-settings
sed -i "s/DISTRIB_DESCRIPTION='LEDE '/DISTRIB_DESCRIPTION='OpenWrt_1806_x64_主路由版 by GXNAS build '/g" package/lean/default-settings/files/zzz-default-settings
...
sed -i "s/<%=pcdata(ver.distname)%> <%=pcdata(ver.distversion)%>/OpenWrt_1806_x64_主路由版 by GXNAS build @R$(date +%y.%m.%d)/g" package/lean/autocore/files/x86/index.htm
cat package/lean/autocore/files/x86/index.htm
