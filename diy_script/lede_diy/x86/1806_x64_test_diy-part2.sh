...
sed -i "s/<%=pcdata(ver.distname)%> <%=pcdata(ver.distversion)%>/<%=pcdata(ver.distname)%> <%=pcdata(ver.distversion)%> OpenWrt_1806_x64_测试版 by GXNAS build @R$(date +%y.%m.%d)/g" package/lean/autocore/files/x86/index.htm
